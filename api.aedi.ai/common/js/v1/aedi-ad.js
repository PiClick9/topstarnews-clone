  ...
  Aedi.prototype = {
    ...
    adCreate: function (api_key, img, attr, date, img_cap, cap = null) {
      ...
      function bottedge(img, ad_HTML, overlapPx = 0) {
        ...
        iframe.classList.add('bottedge-ad');
        iframe.setAttribute('scrolling', 'no');
        iframe.setAttribute('allowtransparency', 'true');
      
        const imgCS = getComputedStyle(img);
        const picCS = pictureEl ? getComputedStyle(pictureEl) : null;
      
        const imgMb = parseFloat(imgCS.marginBottom) || 0;
        const picMb = picCS ? (parseFloat(picCS.marginBottom) || 0) : 0;
      
        const attachOffset = -(picMb) - (imgMb) - 41 - (overlapPx || 0);
        ...
        const updateBox = () => {
          ...
          const imgCS2 = getComputedStyle(img);
          const picCS2 = pictureEl ? getComputedStyle(pictureEl) : null;
          const imgMb2 = parseFloat(imgCS2.marginBottom) || 0;
          const picMb2 = picCS2 ? (parseFloat(picCS2.marginBottom) || 0) : 0;
      
          wrap.style.marginLeft = imgCS2.marginLeft;
          wrap.style.marginRight = imgCS2.marginRight;
      
          // picture 태그가 있으면 30, 없으면 41
        ...
        if ('ResizeObserver' in window) {
          new ResizeObserver(updateBox).observe(img);
        } else {
