			...
			switch(type){
				...
				/*if(state == '무제한') {
					...
					var _time = Math.floor(new Date().getTime()/1000)
						,_banid = _time + '_' + bannerIdxno
						,_html = '<!--'+title+'-->'
							   + '<script type="text/javascript">var ___BANNER = "ban_'+_banid+'";</script>'
							   + '<script type="text/javascript" charset="utf-8" src="/bannerpop/uploads/js/'+bannerIdxno+'.js?'+_time+'" id="ban_'+_banid+'"></script>'
							   + '<!--//'+title+'-->';
				//}
				...
				case "sourceCopy_activity":					// 소스복사
					var _time = Math.floor(new Date().getTime()/1000)
						...
						,_html = '<!--'+title+'-->'
							+ '<script type="text/javascript">var ___BANNER = "ban_'+_banid+'";</script>'
							+ '<script type="text/javascript" charset="utf-8" src="/bannerpop/uploads/js_activity/'+bannerIdxno+'.js?'+_time+'" id="ban_'+_banid+'"></script>'
							+ '<!--//'+title+'-->';
                ...
                case "sourceCopy_multi":
                    var _time = Math.floor(new Date().getTime()/1000)
						,_banid = _time + '_' + bannerIdxno
                    	,_html = '<!--'+title+'-->'
								+ '<script type="text/javascript">var ___BANNER = "ban_multi_'+_banid+'";</script>'
								+ '<script type="text/javascript" charset="utf-8" src="/bannerpop/uploads/js_multi/'+bannerIdxno+'.js?'+_time+'" id="ban_multi_'+_banid+'"></script>'
								+ '<!--//'+title+'-->';
                    adWrite.copyExe(_html);
                    break;
			}
...
var bannerpop={
	...
	* json 내용 *
		...
		bottom_close_expire_ck - **동안 열지않기 (none=사용안함, day=하루동안 열지않기, days=며칠동안 열지않기)
		bottom_close_expire - 며칠 동안 인지
	...
	*****/
				...
				if($thisBtn.is(".popup-days")){
					...
					// 며칠동안 daySubfix
						...
						var html = '<a href="#link">'
									+ (period>1 ? '며칠동안 보지 않기' : '하루동안 보지 않기')
									+'</a>';
	...
	,popupControl:function()
	{
		...
		var parents = this
			...
			,bottomCloseExpire = p.bottom_close_expire||1	// : 며칠
                ...
                agent.forEach(function(agt) {
                    try{
                        ...
                        if (agt=="gtm" && typeof dataLayer!="undefined") {
                            dataLayer.push({'event':'NDVIEWBANNER','event_category':'배너뷰', 'event_label':banner});
                        }
            ...
            case "click":
                agent.forEach(function(agt) {
                    try {
                        ...
                        if (agt=="gtm" && typeof dataLayer!="undefined") {
                            dataLayer.push({'event':'NDCLICKBANNER','event_category':'배너클릭', 'event_label':banner});
                        }
                        if (agt=="wcsnaver" && typeof wcs!="undefined") {
        ...
        $('.banner-multi-box').each(function() {
            ...
            if ($effectType!="none") {
                var totalSlide=$(this).find(".slick-slide").length||0,
					slideCount=$effectType=="horizon"?(totalSlide/Number($rowNum))||0:totalSlide,
					randomStart=Math.floor(Math.random()*slideCount);
...
function resizePhoto() {	
	...
	$_container.find('img').each(function(e){
		if($(this).closest('figure').length) {
			...
			var imgOrg = jQuery(this).data("org");
			ViewimageObj.src = $(this).attr("src");
			ViewimageObj.onload = function() {
				imageWidth = this.width;
				if(articleViewDivWidth <= imageWidth || imgOrg) {
	...
	$_container.on('click', 'figure.bigsize', function(e){
		var imgSrc = jQuery(this).find('img').attr('src'),
			imgOrg = jQuery(this).find('img').data('org');
		
		if(imgOrg) imgSrc = imgOrg;
		...
		$('<form action="/news/userArticlePhoto.html" method="POST"><input type="hidden" name="src" value="'+imgSrc+'"></form>').appendTo('body').submit();
	})
